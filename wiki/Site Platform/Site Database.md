...
- **Engaged time is a delta-per-hide, not a heartbeat stream** (#64). Because rows can't be UPDATEd, the client accrues activity-gated engaged ms locally and flushes one `meta`/`engaged_time` event on each tab-hide; SQL sums the deltas. Same fidelity as 15s heartbeats at ~10× fewer rows. Bot/dev traffic is flagged on the visit row (`device.bot`, `device.dev`) and filtered by joining events on `session_id` — localhost never sends at all.
